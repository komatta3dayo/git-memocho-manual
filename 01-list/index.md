...
- ファイルタイプ変更  
  選択したメモのファイルタイプを変更します。  
  - テキスト テキスト形式のファイル。メモ編集・参照画面ではテキスト形式のファイルとして表示されます。
  - Markdown Markdown形式のテキストファイル。メモ編集・参照画面ではMarkdownファイルとして表示されます。
  - その他 その他のファイル形式。メモ編集・参照画面では開けません。外部のアプリで表示します。
...
- 直前のコミットに戻す。
  ファイルがコミットされていない状態の場合、直前のコミット時点の状態に戻します。
...
- バージョン管理  
  ...
  - git commit  
    ...
    手動コミットの場合場合はメモ更新後のメモタイトルに`?`とマークがつきます。  
...
- OpenAI  
  設定画面でOpenAI API機能を有効にした場合に表示されます。  
  - OpenAI API chat  
    OpenAI APIを用いたチャットを実行します。チャットの結果はテキストファイルに保存できます。
  - OpenAI edit  
    OpenAI APIを用いたファイル編集機能です。  
    編集対象のファイルを選択して、編集指示を出すと編集結果が表示されます。  
  - OpenAI image creation  
    OpenAI APIを用いた画像生成機能です。  
    
- インポート  
  PDFからテキストを抽出、Wordからテキストを抽出、ExcelからCSVデータを抽出するメニューを選択するダイアログを表示します。
  - PDFからテキストを抽出  
    PDFからテキストを抽出を抽出してメモを作成します。
  - MS Wordからテキストを抽出 
    Wordからテキストを抽出を抽出してメモを作成します。  
  - Excel  
    Excelからテキストを抽出を抽出してメモを作成します。  
